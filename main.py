...
class MentalHealthChat(QWidget):
    ...
    def save_conversation(self, text):
        if self.session_id:
            filename = f"session_{self.session_id}.txt"
            with open(filename, "a", encoding="utf-8") as f:
                f.write(text + "\n")

    def set_session_id(self):
        new_id = self.session_input.text().strip()
        if new_id:
            ...
            try:
                ...
                self.conversation.append(f"\n🟢 Loaded previous conversation for session {self.session_id}\n")
            except FileNotFoundError:
                self.conversation.append(f"🟢 Session ID set: {self.session_id}. No previous conversation found.\n")
        else:
            self.error_box("Please enter a valid session ID.")
    ...
    def init_ui(self):
        ...
        session_label = QLabel("Enter Session ID:")
        session_label.setStyleSheet("color: #FFE66D; font-weight: bold;")
        self.session_input = QLineEdit()
        self.session_input.setPlaceholderText("Paste your session id here")
        self.session_input.setStyleSheet("background: #B0FFF7; color: #1E3557; border-radius: 8px;")
        session_set_btn = QPushButton("Set Session")
        session_set_btn.setStyleSheet("background-color: #FFE66D; color: #1E3557; border-radius: 8px;")
        session_set_btn.clicked.connect(self.set_session_id)
        session_layout.addWidget(session_label)
        session_layout.addWidget(self.session_input)
        session_layout.addWidget(session_set_btn)
    ...
    def send_message(self):
        ...
        self.save_conversation(f"You: {user_text}")
        ...
        try:
            ...
            if resp.ok:
                ...
                self.save_conversation(f"Companion: {companion_msg}")
            else:
